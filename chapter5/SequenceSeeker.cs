...
public class SequenceSeeker<ElementType> where ElementType: MusicalElement
{
	...
	public void ProceedTime(float deltaBeatCount)
	{
		...
		// 現在時刻の直後にあるマーカーのインデックスを取得する.
		int		index = find_next_element(m_nextIndex);

		//「直後のマーカー」と「シーク位置」が等しくない？.
...
//private variables
	private int		m_nextIndex         = 0;		//シーク位置（＝現在時刻からみて、次にあるエレメントのインデックス）.
	private float	m_currentBeatCount  = 0;		//現在時刻.
	private bool	m_isJustPassElement = false;	//シーク位置が進んだフレームのみ、true になる.

	private List<ElementType> m_sequence;			//シークするシーケンスデータ.
}
